...
**Filled by:** operator during the soak (CC-1Ad); five fields cannot be
reconstructed after the fact — missing any one voids the *report*, not the run.

| # | Field | Placeholder | Notes |
|---|---|---|---|
| 1 | Process start timestamp (UTC) | `_TBD_` | Wall-clock when the running `chain` binary started |
| 1b | Git SHA of the running binary | `_TBD_` | Pin from `cc_build_info` / `CC_GIT_SHA`; a mid-run binary swap voids the run |
| 2 | Catch-up complete timestamp (UTC) | `_TBD_` | From `cc_driver_catchup_complete_timestamp` (CC-1C/3). Opens the steady-state window. **Not** hand-entered when the report script runs |
| 3a | Hour-2 RSS (KiB) | `_TBD_` | Clause 2/5 baseline — earliest health read |
| 3b | Hour-24 RSS (KiB) | `_TBD_` | Clause 2/5 end-of-window pair |
| 4a | Driver (block-feed) provider | `_TBD_` | The provider `bin/driver` polls for blocks |
| 4b | Independent reference provider | `_TBD_` | Head-agreement sampler target; **must differ** from 4a (Clause 2/2). Enforced by `scripts/soak-sampler.sh` |
| 5 | Machine spec | `_TBD_` | CPU / cores / RAM / OS; plus every provider rotation and gap event with a timestamp |
| 6 | Per-slot load/RSS series | `_TBD path → soak-samples.csv_` | Output of `scripts/soak-sampler.sh` — input to R-1's load-spike guard |

### Events log (rotations / gaps)

| Timestamp (UTC) | Event | Detail |
|---|---|---|
| `_TBD_` | `_none yet_` | e.g. provider rotation, `cc_driver_gap_abandoned_total` increment |
...
| Field | Value |
|---|---|
| Block bucket fraction at `le=0.4` | `_TBD_` (Δbucket / Δcount) |
| Block budget | p95 ≤ 0.4 s ⇔ fraction ≥ 0.95 |
| Block margin (fraction − 0.95) | `_TBD_` |
| Block pass | `_TBD_` |
| Epoch bucket fraction at `le=1.0` | `_TBD_` (Δbucket / Δcount) |
| Epoch budget | p95 ≤ 1.0 s ⇔ fraction ≥ 0.95 |
| Epoch margin (fraction − 0.95) | `_TBD_` |
| Epoch pass | `_TBD_` |
| Measurement window (steady-state) | `_TBD_ → _TBD_` |
| Excluded catch-up window | `_TBD_ → catch-up complete_` (from `cc_driver_catchup_complete_timestamp`) |
| Head-agreement (steady) | `_TBD_` |
| Driver provider | `_TBD_` |
| Reference provider | `_TBD_` |
| RSS hour-2 / end (KiB) | `_TBD_ / _TBD_` |
| R-1 load guard | `_TBD_` (refuse if sustained spike in window) |
| Overall verdict | `_TBD PASS/FAIL_` |
...
redesign — and the filled report must say so explicitly.
