...
//! - **CC-2D**: `syncnets` empty-by-default, Phase 6 `subscribe_sync_subnets` hook, sync validators
...
//! - **CC-26a / CC-48**: bounded backfill cache (1 GiB ceiling); advertised
//!   `earliest_available_slot` AtomicU64 written only by `WatchServeWindow`
