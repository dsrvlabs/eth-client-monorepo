...
| 2 | **The `cc-seam` conformance suite is "11 tests"; `[ARCH]` §2.2 names 4.** Seven are unspecified in either source | 5–7 pd sized against an unenumerated list | `S1-A-12` enumerated 11 named cases in `[ARCH]` §2.1 (1–8 share one assertion on both impls; case 9 is an impl split; B and D stay in `cc-chain`). Count is honest |
