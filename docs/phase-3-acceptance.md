...
| Member count (CC-3K /8) | `cargo metadata --format-version 1 --no-deps \| jq '.packages \| length'` | Phase 3 close: **16** (not 17 — CC-28 retired `bin/driver` 17→16 before Phase 3). Phase 3 **adds no workspace member** (ADR P3-01 / ≠13/1). **Today: 23** after later-phase admissions (Phase 4 store DAG 16→19; CC-4J 19→20; S0-A-13 20→21; S1-A-01 21→22; S1-A-07 22→23). Phase-3-close names: `cc-attestation cc-beacon-api cc-bootstrap cc-chain cc-config cc-crypto cc-devnet-gen cc-engine cc-fork-choice cc-libp2p cc-p2p cc-proto cc-spec-tests cc-state-transition cc-storage cc-types`. Today: `cc-attestation cc-beacon-api cc-bootstrap cc-chain cc-config cc-crypto cc-devnet-gen cc-engine cc-engine-api cc-fork-choice cc-libp2p cc-p2p cc-proto cc-scheduler cc-seam cc-serve-probe cc-spec-tests cc-state-transition cc-storage cc-store cc-store-bench cc-store-tool cc-types` |
