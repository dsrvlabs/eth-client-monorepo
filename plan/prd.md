...
**`Discharged by` (M3).** The column on §5.1 and §5.2 is the tracking artifact `[PLAN]` E0.9 requires
and that `S0-B-18` creates. A cell holds a **12-hex commit SHA** (optionally `SHA → Sn`) when the row
was patched, or a **stage id** when it was discharged by deletion. Unlanded rows this stage claimed
may carry an owning issue id that exists as a `### \`S0-…\`` heading under `plan/issues/` (`S0-B-13`)
until the SHA lands — not a blank, not `TBD`, not `pending this stage`, not a free-form id.

**Zero blank cells for rows the current stage claimed.** Asserted by `scripts/check-m3-discharged-by.sh`,
wired into the `S0a-B-04` `make ci` gate list. A blank claimed cell is a CI failure, not a review note.

Each stage exit carries a ~0.5 pd line item to update the column. That is the program's
*no-P0-vanishes-silently* check: the issue also appends its stage to the script's `CLAIM_STAGES` so the
next wave cannot land blank.
...
| --- | --- | --- | --- |
| 1 — `top_up_pubkey_cache` + 3 call sites | **S** (~30 lines + 3 calls) | `patch @ S0` | `358da5f56f55` |
| 1b — `from_ssz_bytes_hydrated` chokepoint | **S** (~1 day) | `patch @ S0` — makes the class unrepresentable rather than fixed-thrice | `738864baaf2a` |
| 2 — round-trip regression test, hand-fill forbidden | **S** (~1 day) | `patch @ S0` | `fde1fa9e6718` |
| 3 — move the cache off `BeaconState` onto `TransitionContext` | **M** (1–2 weeks) | `patch @ S2` — **prerequisite for P1-D/10**, see below | `S2-A-10` |
| 4 — persist in `cc-store` | **S–M** | `patch @ S2`, optional | `S2-A-10` (optional persist; not scheduled) |
...
| --- | --- | --- | --- | --- |
| 1 | `services/storage/src/serve.rs:821` | `PutBackfillBatch` can overwrite the canonical index at any slot: no anchor/frontier binding, progress optional. Server-side validation for [RV] Vuln 2. | `patch @ S0` | `13f9d5f8239b` |
| 2 | `services/storage/src/backfill.rs:219` | Admission never checks the batch attaches to the durable frontier; `blocks_oldest` can jump down across a hole | `patch @ S2` | `S2-B-07` |
| 3 | `services/storage/src/backfill.rs:87` | `per_index` padding fabricates progress for never-custodied indices; monotone guard rejects honest cgc-raise reports | `patch @ S2` | `S2-B-08` |
| 4 | `services/storage/src/write_behind.rs:214` | Panic respawn resubscribes with the boot-time cursor, discarding committed progress | `deleted @ S2` | `S2` |
| 5 | `services/storage/src/main.rs:352` | CC-4A block floor read from a build-machine fixture path with silent hardcoded fallback + `unwrap_or(0)` | `patch @ S0` | `692d2bfcd993` |
| 6 | `services/storage/src/prune/mod.rs:532` | `drop_table` failure after marks durably advanced leaks the stripped shard permanently | `patch @ S2` | `S2-B-09` |
...
| 8 | `services/p2p/src/gossip/validate/pipeline.rs:897` | Clock disparity quantized up to a full slot — widens all gossip timeliness checks **24×** | `patch @ S0` | `S0-B-13` |
| 9 | `services/p2p/src/gossip/validate/column.rs:445` | Column validator omits two spec REJECT conditions (slot > parent slot; finalized-ancestor) | `patch @ S0` | `S0-B-13` |
| 10 | `services/p2p/src/gossip/registry.rs:581` | `advance_to` requires exact epoch equality; a missed epoch tick **permanently wedges subscriptions** | `patch @ S0` | `S0-B-13` |
...
| 12 | `services/p2p/src/reqresp/blocks.rs:446` | Empty in-window results answered with error code 3 instead of an empty success stream | `patch @ S0` | `S0-B-15` |
| 13 | `services/p2p/src/reqresp/columns.rs:536` | Column by-range chunks served in request order, not ascending `(slot, column_index)`; comment contradicts code | `patch @ S0` | `S0-B-16` |
...
| 18 | `crates/state-transition/src/block/mod.rs:160` | Sync-aggregate signature verified even under `NoVerification` strategy | `patch @ S0` | `S0-A-33` |
| 19 | `crates/fork-choice/src/on_attestation.rs:284` | `update_latest_messages` partially mutates vote trackers on OOB index, skips the counter bump | `patch @ S0` | `67d9f1aca953` |
| 20 | `crates/fork-choice/src/on_block.rs:452` | Justified `CheckpointContext` overwritten from the importing block's post-state, not the checkpoint state | `patch @ S0` | `0a80501e1859` |
| 21 | `crates/fork-choice/src/head_cache.rs:151` | `get_proposer_head` has an invented equivocation branch that bypasses the spec's safety conditions | `patch @ S0` | `5ce8ff9899aa` |
| 22 | `services/chain/src/restore.rs:189` | `RestoreGate::end_stream` never notifies waiters — a failed restore after grace **hangs bootstrap forever**. **⚠ P0-19 is a candidate common cause**: restore decodes at `:437` and feeds `on_block` at `:525` on a state with an empty pubkey cache, so every replayed block returns `CachePoisoned` — which is one explanation for *why* the restore fails before this handler's gap matters. **Test that hypothesis before S2 deletes the surface** (§10/J-14). | `deleted @ S2` — **but diagnose first** | `S2` |
| 23 | `services/chain/src/restore.rs:656` | Restore silently drops DA-deferred blocks — `DataAvailable` can never re-drive them. **⚠ Same candidate common cause as /22.** | `deleted @ S2` — **but diagnose first** | `S2` |
| 24 | `services/chain/src/da.rs:395` | Block-branch `FetchBlobs` template always carries a zeroed KZG inclusion proof in production | `patch @ S1` | `S1-B-03` |
| 25 | `services/engine/src/main.rs:135` | Production binary passes `kzg: None` — the entire CC-37b getBlobs fastpath is dead | `patch @ S1` (links P0-16) | `S1-B-01` |
| 26 | `services/engine/src/main.rs:133` | Production lane uses the `hoodi_blob_bound()` **test fixture** for the blob-count gate | `patch @ S1` | `S1-B-02` |
| 27 | `docker-compose.yml:127` | Storage can never enforce I-node-id: no identity mount, no `CC_STORAGE_NODE_KEY_PATH` in compose | `patch @ S0 → deleted @ S2` | `8c9089fb0c07` → S2 |
| 28 | `.github/workflows/ci.yml:208` | Proto breaking baseline hardcodes `branch=develop` for pushes, but the workflow also runs on push to `main` | `patch @ S0` | `00862a5b9e07` |
| 29 | `devnet/faults.sh:27` | Pinned to `devnet/compose.yml`, but the CC-4N kill-9 clause documents it against the main stack | `patch @ S0` | `d9fe29f43d1f` |

#### P1-B — Medium quality line items (12) · [RV] §3

| # | Location | Requirement | Disposition | Discharged by |
| --- | --- | --- | --- | --- |
| 1 | `services/storage/src/write_behind.rs:599` | `observe_lag` runs on the already-reset accumulator, so `write_behind_lag_slots` **never records** | `patch @ S0` | `56f569def14d` |
| 2 | `services/storage/src/serve.rs:484` | Unary serve permits released at handler return, so the documented 256 MiB ceiling isn't enforced | `patch @ S2` | `S2-B-10` |
| 3 | `services/storage/src/replay.rs:354` | `measure_load_from_store` runs multi-second CPU work (200 MB SSZ decode + tree-hash) on the async runtime thread | `patch @ S2` | `S2-B-11` |
...
| 8 | `services/p2p/src/gossip/validate/column.rs:707` | Fork-version schedule walk hand-duplicated across three validator files | `patch @ S4` (links P1-D/15) | `7e6d09796f6a` (at S0; `@ S4` superseded by C-11) |
| 9 | `services/p2p/src/storage_client.rs:240` | `invalidate()` flips `available=false` with no data-plane path to restore it; one transient RPC error refuses serves until the watch stream speaks | `deleted @ S2` | `S2` |
| 10 | `crates/state-transition/src/epoch/mod.rs:53` | `block_to_epoch` catch-all relabels every unexpected error as `ArithmeticOverflow` | `patch @ S0` | `ffc32cdfd83d` |
| 11 | `services/chain/src/engine_client.rs:177` | Core thread blocks on engine gRPC with no connect/RPC deadline | `patch @ S0` (= P0-15) → `deleted @ S1` | `9610afe65a95` → S1 |
| 12 | `crates/crypto/src/bls/batch.rs:127` | `SignatureSet` batch verification bypasses the `BLS_VERIFY_COUNT` instrumentation contract | `patch @ S0` | `S0-A-35` |

#### P1-C — Convention (1) · [RV] §3

| # | Location | Requirement | Disposition | Discharged by |
| --- | --- | --- | --- | --- |
| 1 | `.github/workflows/ci.yml:49` | All GitHub Actions pinned by mutable tag (`@v4`…), contradicting the repo's own exact-pin policy (`docs/supply-chain.md`). Pin to commit SHAs. | `patch @ S0` | `5868047620c2` |

#### P1-D — Cross-cutting programs (10) · [AS] §4 Med findings 09–18

| ID | Program | Subsumes | Disposition | Discharged by |
| --- | --- | --- | --- | --- |
...
| 11 | **Stringly-typed cross-service protocols ✳** — verdict reason strings, `FCU_DROPPED_STALE:` prefixes, hand-rolled event byte-offsets with silent-default fallbacks. **Type the contracts before moving them** (see §8/R-1). | — | `patch @ S1–S2` (before transport moves) | `S1-A-14` (S1 half; `S2-A-08`) |
...
| 13 | **Chain event bus doubles as a bulk data plane** — full column sidecars relayed through a 64 MiB ring couple archive durability to another process's eviction timing; gap-fill can invent canonical roots. | — | `deleted @ S2` | `S2` |
| 14 | **Graceful shutdown is largely unreachable or lying ✳** — storage's shutdown watch never fires; biased selects hot-spin on `watch` Err; drain-timeout exits 0. | — | `patch @ S0` (fire the watch) → `deleted @ S2` | `21b8d759c082` → S2 |
...
| 17 | **Governance / QA drift ✳** — (a) the ADR/Architecture corpus cited **~745×** across **~59 distinct ADR ids** does not exist in the repo ✓ᴸ — **import or re-derive it as a gate on Stage 2 entry** (D-6). **At this size it needs its own estimate**, and the reconciliation mechanism that makes it tractable is itself unwritten (§10/J-16); (b) Makefile doesn't mirror CI. (The red-CI-gate half is P0-08.) | P2-B/8 (`Makefile:193`), **promoted to P0-08(d)** | (a) **gate on S2 entry — size it first**; (b) `patch @ S0` | `a299fd96f019` (b); (a) S2 entry |
...
| **P2-E** | 35 | [RV] §4 unverified — **enumerated in §5.3.2** | **triage pass first** — 35 judged (`S0-B-17` × 5 + `S1-B-19` × 17 + `S1-B-20` × 13, 2026-08-16); **M12 = 0** |
...
| 3 | `crates/state-transition/src/block/operations/deposit.rs:119` | `apply_deposit` reimplements `get_validator_index_by_pubkey`, bypassing scan accounting | with P0-02 (`c6c5fd0bc7bd`) |
...
| 8 | `Makefile:193` | `make deps/lint/ci` do not mirror the CI gates they claim to; a guard-script header claims wiring that does not exist | **promoted to P0-08(d)** (`a299fd96f019`) |
...
| 22 | `services/storage/src/prune/mod.rs:57` | V | Blanket `#![allow(dead_code)]` on three production modules masks real dead code. **`S1-B-20`: promoted P2 `patch @ S2`** (live site `:57`) |
| 23 | `services/storage/src/columns.rs` / `crates/store/src/columns.rs:517` | Q | `columns_for_block` carries a dead `seen` bitmap. **`S1-B-20`: dismissed** (`seen` unreachable on the `0..NUMBER_OF_COLUMNS` walk; storage copy gone) |
| 24 | `services/p2p/src/reqresp/columns.rs:549` | Q | Dead requested/returned index tracking with a false comment, plus doc-comment debris. **`S1-B-20`: dismissed** (live `:504-552`; leftover locals; serve already fail-closes) |
| 25 | `services/p2p/src/clock.rs:168` | Q | `spawn_epoch_ticks` comment claims sub-slot polling but the period is exactly one full slot. **`S1-B-20`: promoted P2 `patch @ S3`** (live site `:264-266`) |
| 26 | `services/p2p/src/clock.rs:202` | V | Test modules use outer `#[allow]` instead of the prescribed inner `#![allow]`. **`S1-B-20`: dismissed** (live `:308-310`; equivalent lint scope) |
| 27 | `services/p2p/src/discovery/task.rs:559` | Q | Discovery loop awaits `find_node_predicate` queries inline, stalling shutdown / epoch ENR updates / event draining. **`S1-B-20`: promoted P1 `patch @ S3`** (live site `:560-614`) |
| 28 | `services/p2p/src/chain_stream/client.rs:715` | Q | Publish forward drop is silent; the comment describes retry/accounting logic that does not exist. **`S1-B-20`: promoted P1 `patch @ S3`** (live site `:709-717`) |
| 29 | `services/p2p/src/das/custody.rs:171` | Q | `CustodyManager::new` `# Panics` doc contradicts the silent `min()` clamp. **`S1-B-20`: dismissed** (live `:170-176`; clamp is the live contract) |
| 30 | `services/p2p/src/das/sampling.rs:679` | V | Test modules use outer `#[allow]` instead of inner `#![allow]` (`sampling.rs`, `inject.rs`, `server.rs`). **`S1-B-20`: dismissed** (live `sampling.rs:666-668`, `engine_stream/inject.rs:776-778`, `engine_stream/server.rs:314-316`) |
...
| 32 | `bin/serve-probe/src/probe.rs:407` | Q | `negative_verdict` labels a non-empty success chunk as "empty success". **`S1-B-20`: dismissed** (live `:405-409`; verdict is correct, reason string only) |
| 33 | `crates/bootstrap/Cargo.toml:34` | V | Dev-dependency `serde_json` pinned inline instead of `workspace = true`. **`S1-B-20`: promoted P2 `patch @ S1`** (live site `:34`) |
| 34 | `bin/devnet-gen/src/genesis.rs:167` | Q | GVR helper swallows list-overflow into an empty-list root; dead BLS-creds block above. **`S1-B-20`: dismissed** (live `:48-56` / `:165-166`; overflow unreachable after `validators_push`) |
| 35 | `crates/types/src/sidecar/mod.rs:155` | Q | `empty_bitlist` helper duplicated verbatim in `operations.rs` and `sidecar/mod.rs`. **`S1-B-20`: dismissed** (verbatim private `Default` helpers; no divergence) |
...
Rows **22–30, 32–35** were judged at `S1-B-20` (2026-08-16). **M12 = 0.**
...
##### S1-B-20 triage — 2026-08-16 · M12 13 → 0 unknown

Adversarial pass over rows 22–30, 32–35 (**13** rows). The work-item's "14" counted
row 31, already judged at `S0-B-17` and **not re-opened**. Line numbers below are
the live sites on `develop` `4b3eff0` (several have drifted from the [RV]
citations). Promoted rows become follow-up issues against their owning stage and
are **not** added to the P0=19 / P1=58 counts here (same rule as `S0-B-17` /
`S1-B-19`). This write does not ship those patches. **`S1-B-19`'s rows (2–5, 7,
9–20 — 17) remain unknown** — this write does not judge them.

| # | Live site | Judgement | Tier | Disposition | Reason |
|---|---|---|---|---|---|
| **22** | `services/storage/src/prune/mod.rs:57` | **promoted** | P2 (V) | `patch @ S2` | Still a file-wide `#![allow(dead_code)]` on the production prune module, inherited by all five children (`blocks`, `chunk`, `columns`, `shards`, `states`). Unlike `writer.rs:10` this allow has **no rationale**. It is doing work: `:1168-1173` adds a dummy `_epoch_start_slot_anchor` just to keep an unused import quiet, and `shards.rs:232-238` / `states.rs:75` pile on more per-item allows underneath. Same *class* as P2-C (production-wide allow masking defects) but `dead_code`, not `unwrap`. Storage fold is the cleanup window. Not patched in this PR. |
| **23** | `crates/store/src/columns.rs:521-542` | **dismissed** | — | — | `seen` is dead. The walk is `0..NUMBER_OF_COLUMNS` once; `want[]` already dedups `requested`. The `seen[i]` guard is therefore never true on this loop, then `seen[i] = true` is never read back. `services/storage/src/columns.rs` **does not exist** on this tree — the helper lives only in `cc-store`. Emission is still index-ascending and missing indices still go to `missing`. Residual is three leftover lines, not a live invariant. |
| **24** | `services/p2p/src/reqresp/columns.rs:504-552` | **dismissed** | — | — | `requested_indices` / `returned_indices` are still pushed then discarded (`let _ = (requested_indices, returned_indices)`). The comment *"used by tests"* is false (no test reads those locals). Serve already fail-closes: a missing requested column returns `ResourceUnavailable` and never silently omits. Leftover unused vecs + a stale comment, not a live multiset check. Opportunistic delete, not scheduled. |
| **25** | `services/p2p/src/clock.rs:264-266` | **promoted** | P2 (Q) | `patch @ S3` | `spawn_epoch_ticks` (now `:259`) still says *"Poll on a sub-slot cadence so epoch edges are not delayed by a full slot"* and then sets `period = Duration::from_secs(seconds_per_slot.max(1)).max(Duration::from_millis(50))`. That is **one full slot** (12 s on mainnet); `.max(50ms)` cannot produce a sub-slot once `seconds_per_slot ≥ 1` (already clamped in `SlotClock::new`). Consumers are discovery ENR apply (`discovery/task.rs:507`) and status re-exchange (`service.rs:562-595`) — a 1-slot lag, not a skipped epoch. Not P1-A/10 (that wedge was exact-epoch `advance_to`, already patched). Fix the combinator or the comment. Not patched in this PR. |
| **26** | `services/p2p/src/clock.rs:308-310` | **dismissed** | — | — | Still `#[cfg(test)] #[allow(clippy::unwrap_used, clippy::expect_used)] mod tests`. `docs/dev-conventions.md` asks for an inner `#![allow]`. Outer `#[allow]` on the test module has the **same lint scope**; production code stays under the workspace lint. Style-only; not P2-C (that is a file-wide allow on *production* `fault_mode.rs`). |
| **27** | `services/p2p/src/discovery/task.rs:560-614` | **promoted** | P1 (Q) | `patch @ S3` | The `select!` loop (`:498`) is the only pump for shutdown, `epoch_rx` ENR updates, and `event_stream`. Both `run_generic_query` (`:560`) and every `run_attnet_query` (`:591`) `.await` `discv5.find_node_predicate` **inline** in the `query_interval` arm — no timeout in our wrapper (`:372-395`). Generic + sequential deficit-subnet queries therefore stall the whole task for the life of the DHT walks (5 s cadence below target, 30 s subnet cooldown, `QUERY_TARGET_PEER_NO = 16`). Shutdown, fork-boundary `eth2`/`nfd` apply, and `Discovered`/`SessionEstablished` draining all wait. P1-B/7-shaped (inline await stalls a critical loop) @ S3, not P1-A/11 (that is persisted ENR seq). Drive queries off-loop or budget them. Not patched in this PR. |
| **28** | `services/p2p/src/chain_stream/client.rs:709-717` | **promoted** | P1 (Q) | `patch @ S3` | `Publish` still `try_send`s onto `publish_fwd_tx` (`mpsc` bound `PUBLISH_BOUND = 256`) and on `Err` only `warn!`s. The comment (`:713-716`) describes a short-budget retry and *"count a drop here if channel closed"*; neither exists. `run_publish_dispatch` (`publish.rs:58-147`) is the path that oldest-drops **loudly** (`error!` + `PublishDropCounter`). A full or closed forward channel drops the proto `PublishRequest` **before** that dispatcher, so the loud counter never moves. Local publishes are documented as loud (CC-27b). Wire the retry/count or drop through `enqueue`. Not patched in this PR. |
| **29** | `services/p2p/src/das/custody.rs:170-176` | **dismissed** | — | — | `# Panics` still claims `custody_group_count > NUMBER_OF_CUSTODY_GROUPS` panics via `get_custody_groups`. `new` does `custody_group_count.min(NUMBER_OF_CUSTODY_GROUPS)` **first**, so the spec assert (`crates/types/src/networking/custody.rs:34-37`) is never reached from this constructor. The same `min()` is the live contract on peer coverage (`:286`), recovery, and ENR **read** (`enr.rs:693` `enr_custody_groups`); `encode_cgc` does not clamp. `cgc_hook.rs` is the fail-closed surface (`CgcOutOfRange`). Residual is a stale doc comment; the clamp is intentional, not a silent spec bypass. |
| **30** | `services/p2p/src/das/sampling.rs:666-668` · `engine_stream/inject.rs:776-778` · `engine_stream/server.rs:314-316` | **dismissed** | — | — | Same convention nit as row 26, still live on all three test modules the [RV] row named (`inject`/`server` sit under `engine_stream/`, not `das/`). Outer `#[allow]` ≡ inner `#![allow]` for a `mod tests`. Not scheduled. |
| **32** | `bin/serve-probe/src/probe.rs:396-410` | **dismissed** | — | — | `negative_verdict` still fails both an empty stream (`:398-401`) and a `ResponseChunk::Success` (`:405-409`). The success-chunk reason string says *"empty success"* even when `ssz_len > 0`. The **verdict is correct** (negative side must be result-byte 3; a success chunk is a fail). Residual is a misleading reason string, not a false pass or a false fail. Probe independence (P0-06 / row 31) is a different row and already discharged. |
| **33** | `crates/bootstrap/Cargo.toml:34` | **promoted** | P2 (V) | `patch @ S1` | Still `serde_json = "1.0.145"` inline. Workspace already pins the same version (`Cargo.toml:133`); every other crate (`engine`, `p2p`, `chain`, `serve-probe`, `devnet-gen`, `state-transition`, `crypto`) uses `workspace = true`. No skew today; the failure mode is the next workspace bump leaving bootstrap behind. Topology-independent one-liner. Not patched in this PR. |
| **34** | `bin/devnet-gen/src/genesis.rs:48-56`, `:165-166` | **dismissed** | — | — | Both halves still present. BLS creds (`creds[0] = 0x00`) are built and discarded (`let _ = creds`); the live `validators_push` uses `eth1_creds` (`0x01` + zero address). `validators_tree_hash_root` still does `VariableList::new(vals).unwrap_or_default()`. That `Err` is **unreachable** here: every validator was already admitted by `validators_push` against the same `P::ValidatorRegistryLimit`, so reconstructing the list cannot overflow. A 0-validator genesis would also `new([])` successfully. Residual dead BLS block + unreachable `unwrap_or_default` are opportunistic hygiene. |
| **35** | `crates/types/src/sidecar/mod.rs:155-160` · `operations.rs:105-110` | **dismissed** | — | — | Both private `empty_bitlist` helpers are still byte-identical (`BitList::with_capacity(0)` + `unreachable!`). Each is used only by its file's `Default` impl. No third copy, no divergence, no public API. Opportunistic DRY, not a schema-duplication row (that class is P1-D/15). Not scheduled. |
...
| **M12** | Unverified findings in an unknown state | 35 | 0 — each promoted with a disposition or dismissed with a reason. **35 judged (`S0-B-17` × 5 + `S1-B-19` × 17 + `S1-B-20` × 13, 2026-08-16).** | Engineering |
