...
pub fn render_scoring_doc(cfg: &ScoringConfig) -> String {
    ...
    s.push_str("<!-- GENERATED by `cc_p2p::gossip::scoring::render_scoring_doc` — do not edit by hand.\n");
    s.push_str("     Source of truth: `services/p2p/src/gossip/scoring.rs` (`build_scoring_config`).\n");
...
mod tests {
    ...
    fn scoring_topic_keys_uses_sparse_column_ids_not_dense_prefix() {
        ...
        assert!(!cols.iter().any(|s| s.ends_with("/data_column_sidecar_0/ssz_snappy")));
        assert!(!cols.iter().any(|s| s.ends_with("/data_column_sidecar_1/ssz_snappy")));
